...
*/
        ...
        scope.onSelectComponent = function (component) {
          ...
          var findComponentSelected = scope.selectedDates.findComponentSelected(scope.componentSelected);
          scope.selectedDates.sortByDate();
          var sd = scope.selectedDates;

            if (sd.length>0) {
              if(!findComponentSelected.haveSelected){
                var lastDate=sd[sd.length-1];
                var dateObject={'year':lastDate.date.getFullYear(),'month':lastDate.date.getMonth(),'date':lastDate.date.getDate(),'day':0,'index':-1,'style':{'isSelected':false,'isToday':false,'index':-1,'isDisabled':false,'isCalendar0':false,'isCalendar1':false,'isCalendar2':false,'isCalendar3':false,'isCalendar4':false,'isCalendar5':false,'isCalendar6':false,'isCalendar7':false,'isViewMonth':true}}
                selectDay(dateObject);
              }
            } else {
              if(!findComponentSelected.haveSelected){
                var date=new Date();
                var dateObject={'year':date.getFullYear(),'month':date.getMonth(),'date':date.getDate(),'day':0,'index':-1,'style':{'isSelected':false,'isToday':false,'index':-1,'isDisabled':false,'isCalendar0':false,'isCalendar1':false,'isCalendar2':false,'isCalendar3':false,'isCalendar4':false,'isCalendar5':false,'isCalendar6':false,'isCalendar7':false,'isViewMonth':true}}
                selectDay(dateObject);
              }
            }
          ...
          if (!isDisabled) {
            ...
            $timeout(function(){
              ...
              if(scope.haveSelectedComponents){
                ...
                for (var i = 0; i < sd.length; i++) {
                  var d=sd[i].date;
                  if (date.year === d.getFullYear() && date.month === d.getMonth() && date.date === d.getDate()) {
                    exist=true;
                    break;
                  }
                }
